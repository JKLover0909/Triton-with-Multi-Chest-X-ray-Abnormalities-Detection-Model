    ...
    def execute(self, requests):
        ...
        for request in requests:
            ...
            num_detections = pb_utils.Tensor(
                "num_detections_3", num_detections.astype(num_detections_dtype))

            detection_boxes = np.array(output_boxes)
            detection_boxes = pb_utils.Tensor(
                "detection_boxes_3", detection_boxes.astype(detection_boxes_dtype))

            detection_scores = np.array(output_scores)
            detection_scores = pb_utils.Tensor(
                "detection_scores_3", detection_scores.astype(detection_scores_dtype))
