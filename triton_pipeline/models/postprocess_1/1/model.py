...
class TritonPythonModel:
    ...
    def execute(self, requests):
        ...
        for request in requests:
            # Get INPUT0
            in_0 = pb_utils.get_input_tensor_by_name(request, "INPUT_1")
            ...
            num_detections = pb_utils.Tensor(
                "num_detections_1", num_detections.astype(num_detections_dtype))

            detection_boxes = np.array(output_boxes)
            detection_boxes = pb_utils.Tensor(
                "detection_boxes_1", detection_boxes.astype(detection_boxes_dtype))

            detection_scores = np.array(output_scores)
            detection_scores = pb_utils.Tensor(
                "detection_scores_1", detection_scores.astype(detection_scores_dtype))
