...
def run_detection(model_name: str, input_image: np.ndarray,
    ...
    outputs.append(grpcclient.InferRequestedOutput(f"num_detections_{num}"))
    outputs.append(grpcclient.InferRequestedOutput(f"detection_boxes_{num}"))
    outputs.append(grpcclient.InferRequestedOutput(f"detection_scores_{num}"))
    outputs.append(grpcclient.InferRequestedOutput(f"detection_classes_{num}"))

    # Test with outputs
    results = triton_client.infer(model_name=model_name, inputs=inputs, outputs=outputs)
    
    num_detections = results.as_numpy(f"num_detections_{num}")
    detection_boxes = results.as_numpy(f"detection_boxes_{num}")
    detection_scores = results.as_numpy(f"detection_scores_{num}")
    detection_classes = results.as_numpy(f"detection_classes_{num}")

    
    return num_detections, detection_boxes, detection_scores, detection_classes


def main(image_path, model_name, url, num_model=2):
    ...
    if prob > 0.3:
        ...
        for i in range(len(num_model)):
            num_detections, detection_boxes, detection_scores, detection_classes = \
                run_detection(model_name, image_det, triton_client, num=i+1)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--image_path', type=str, default='/home/ubuntu/ductq/yte/New folder (12)/1_2_826_0_1_3680043_8_498_3546559833427515712764893873330529826.dcm')
